...
/*   Updated: 2020/12/02 22:40:13 by jaeskim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int precision)
{
		...
		(precision == 0 && (decimal[i] == 9)) || i == 0)
	{
		...
		result = ft_strjoin_free(result, \
			ft_itoa(exp == -1023 ? 0 : integer_len - 1), 3);
	}
	else
	{
		result = ft_strdup(i < 10 ? "-0" : "-");
		result = ft_strjoin_free(result, ft_itoa(i), 3);
	}
	return (result);
}
...
char		*ft_dtoa_e(union u_double n, int precision)
{
	...
	result = ft_strjoin_free(result, \
		ft_dtoa_e_to_str2(decimal, integer_len, \
			n.bit.exponent - (int)FT_DBL_BIAS, precision), 3);
	return (result);
}
